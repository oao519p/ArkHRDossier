...
- 或直接上傳本地 JSON 檔案
- **編隊模式**（預設）：PRTS Wiki 風格卡片，120×250px，裝飾圖層疊加，滿級金色邊框，全技能專三標誌
- **卡片模式**：幹員半身像卡片，包含精英化、等級、潛能、技能、模組
  - 模組切換：預設模組 / 全部模組 / 關閉不顯示
  - 技能切換：預設技能 / 全部技能 / 關閉不顯示
...
- 輸出：
  - **下載 HTML**：快速外連 / 完整離線
  - **下載卡片圖片 ZIP**：每位幹員單張 PNG
  - **下載完整長圖**：格式（JPEG / PNG）× 解析度（1x / 2x）自由選擇
  - **下載 JSON**
...
編隊模式參考 PRTS Wiki 的 half-container 風格，使用裝飾圖層疊加呈現。
...
符合條件的幹員卡片右上角會顯示專三標誌圖示（`./image/specialized_icon/spec3.png`）：
...
可透過控制列「顯示」欄位的**全專精標誌**按鈕開關，僅在編隊模式下出現。

### 輸出支援

編隊模式支援與卡片模式相同的輸出功能：下載 HTML、下載卡片圖片 ZIP、下載完整長圖、下載 JSON。
